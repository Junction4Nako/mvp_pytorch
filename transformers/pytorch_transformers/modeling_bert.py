...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids:Optional[torch.Tensor]=None, position_ids:Optional[torch.Tensor]=None):
    ...
    def __init__(self, config, only_vocab=False):
        ...
        if only_vocab:
            self.decoder = nn.Linear(config.hidden_size,
                                 config.only_word_size,
                                 bias=False)
            self.bias = nn.Parameter(torch.zeros(config.only_word_size))
        else:
            self.decoder = nn.Linear(config.hidden_size,
                                 config.vocab_size,
                                 bias=False)
...
class BertQAPredictionHead(nn.Module):
    def __init__(self, config, only_vocab=False):
        super(BertQAPredictionHead, self).__init__()
        self.transform = BertPredictionHeadTransform(config)
