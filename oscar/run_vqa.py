...
from oscar.modeling.modeling_vlbert import BiImageBertForSequenceClassification, BiImageBertForSequenceClassificationPlus, BiImageBertForVQA
...
MODEL_CLASSES = {
    'bert': (BertConfig, BiImageBertForSequenceClassification, BertTokenizer),
}
    ...
    if name == 'train':
        if args.data_label_type == 'mask':
            if args.use_vg:
                ...
                examples, sgs = processor.get_train_examples(args.txt_data_dir, 'train2014_vg_qla_mrcnn.json')
            else:
                examples, sgs = processor.get_train_examples(args.txt_data_dir, 'train2014_qla_mrcnn.json')
        else:
            examples, sgs = processor.get_train_examples(args.txt_data_dir, 'train2014_qla.json')
    elif name == 'val':
        if args.data_label_type == 'mask':
            if args.use_vg_dev:
                examples, sgs = processor.get_dev_examples(args.txt_data_dir, 'vg_qla_mrcnn.json')
            else:
                examples, sgs = processor.get_dev_examples(args.txt_data_dir, 'val2014_qla_mrcnn.json')
        else:
            examples, sgs = processor.get_dev_examples(args.txt_data_dir, 'val2014_qla.json')
    elif name == 'train+val':
        if args.data_label_type == 'mask':
            examples, sgs = processor.get_train_examples(args.txt_data_dir, 'train+val2014_qla_mrcnn.json')
            #examples = processor.get_train_examples(args.data_dir, 'train+val2014_qla_mrcnn.json')
        else:
            examples, sgs = processor.get_train_examples(args.txt_data_dir, 'train+val2014_qla.json')
    elif name == 'test2015':
        if args.data_label_type == 'mask':
            examples, sgs = processor.get_test_examples(args.data_dir, 'test2015_qla_mrcnn.json')
        else:
            examples, sgs = processor.get_test_examples(args.data_dir, 'test2014_qla.json')
    elif name == 'test-dev2015':
        if args.data_label_type == 'mask':
            examples, sgs = processor.get_test_examples(args.data_dir, 'test-dev2015_qla_mrcnn.json')
        else:
            examples, sgs = processor.get_test_examples(args.data_dir, 'test2014_qla.json')

    return examples, labels, sgs
...
class VQADataset(Dataset):
    ...
    def __init__(self, args, name, tokenizer):
        ...
        self.examples, self.labels, self.phrases = _load_dataset(args, name)
    ...
    def tensorize_example(self, example, phrases, cls_token_at_end=False, pad_on_left=False,
        ...
        tokens_b = self.get_od_labels(example.img_key)
        tokens_b = self.tokenizer.tokenize(tokens_b)

        if self.args.use_b_text:
            # add object tags in sequence a to add more information!
            add_tokensb_b = self.get_od_labels(example.img_key, self.args.b_as_list)
            add_tokensb_b = self.tokenizer.tokenize(add_tokensb_b)
            if len(add_tokensb_b) > num_phrases + self.args.max_seq_length - len(input_ids_a):
                add_tokensb_b = add_tokensb_b[:(self.args.max_seq_length + self.args.max_phrases - len(input_ids_a))]
            else:
                add_tokensb_b = add_tokensb_b
        ...
        if self.args.img_feat_format == 'pt':
            ...
            img_feat = self.get_img_feature(str(example.img_key))
        ...
        if self.c_ids < 1 and 'train' in self.name:
            ...
            if self.args.use_b_text:
                logger.info("add_tokens_b: %s" % " ".join([str(x) for x in add_tokensb_b]))
    ...
    def __getitem__(self, index):
        if self.args.load_fast:
            ...
            phrases = self.phrases[index]
            example = self.tensorize_example(entry, phrases,
    ...
    def get_od_labels(self, img_key, as_list=False):
        if type(self.od_labels[img_key]) == str:
            od_labels = self.od_labels[img_key]
        else:
            if as_list:
                # just use as list
                od_labels = ' '.join(self.od_labels[img_key]['class'])
            else:
                # consider them as set to avoid duplication
    ...
    #for epoch in train_iterator:
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            inputs = {'input_ids_a':      batch[0],
                      ...
                      'labels':         batch[7],
                      'img_feats':      None if args.img_feature_dim == -1 else batch[8]}
            ...
            batch_score = compute_score_with_logits(logits, batch[7]).sum()
            train_score += batch_score.item() / args.train_batch_size
            global_loss += loss.item()
            global_score += batch_score.item()
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:# Log metrics
                    logger.info("Epoch: {}, global_step: {}, lr: {:.6f}, loss: {:.4f} ({:.4f}), score: {:.4f} ({:.4f})".format(epoch, global_step, 
                        optimizer.param_groups[0]["lr"], loss, global_loss / global_step, batch_score.item(), global_score / global_step)
                    )
        ...
        #for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                batch_score = torch.sum(
                    compute_score_with_logits(logits, batch[7]), 1)
                # update results_dict
                results_dict.update(
                    {qa_ind: score for qa_ind, score in
                     zip(batch[9].view(-1).tolist(), batch_score.tolist())}
                )
        ...
        #for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                for i in range(idx.size(0)):
                    ...
                    result = {}
                    result['question_id'] = batch[9][i].item()
...
def main():
    ...
    parser.add_argument('--b_as_list', action='store_true', help="whether to unset the b in text")
    ...
    # to ensure only use tag in seq or in feat
    if args.use_b_feat and args.use_b_text:
        raise NotImplementedError
    ...
    if args.use_single:
        model_class = BiImageBertForSequenceClassificationPlus
    if args.use_pretrain:
        model_class = BiImageBertForVQA
        ...
        if args.eval_all_checkpoints:
            ...
            def filter_epoch(fn):
                ...
                try:
                    ...
                    return False
            checkpoints = [c for c in checkpoints if filter_epoch(c)]
    ...
    if args.do_test and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model = model_class.from_pretrained(checkpoint)
            model.to(args.device)
            res = test(args, model, test_dataset, prefix=global_step)
            with open(checkpoint + ('/{}_results.json'.format(test_dataset.name)), 'w') as fp:
                json.dump(res, fp)
