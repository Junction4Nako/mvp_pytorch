...
from oscar.modeling.modeling_vlbert import BiBertImgForPreTraining
from transformers.pytorch_transformers import (WEIGHTS_NAME, BertConfig,
                                  BertTokenizer)

from oscar.oscar_datasets_ml.build import make_data_loader, make_data_loader_ds
...
MODEL_CLASSES = {
    'bert': (BertConfig, BiBertImgForPreTraining, BertTokenizer),
}
...
def main():
    ...
    if args.visual_learning:
        ...
        model_class = BiBertImgForPreTraining
    load_num = 0
    ...
    for step, (batch, batch_extra) in enumerate(zip(train_dataloader, train_dataloader_extra), start_iter):
        ...
        def forward_backward(image_features, input_ids_a, input_mask_a, segment_ids_a,lm_label_ids_a, 
                            input_ids_b, input_mask_b, segment_ids_b, lm_label_ids_b, phrase_index=None, image_index=None, loss_weight=1.0):
            # feature as input
            ...
            if args.deepspeed:
                ...
                outputs = model_engine(input_ids_a=input_ids_a, token_type_ids_a=segment_ids_a, attention_mask_a=input_mask_a,
                                ...
                                phrase_index=phrase_index, img_index=image_index, max_tag_length=args.max_tag_length)
            else:
                outputs = model(input_ids, segment_ids, input_mask,
                                lm_label_ids, is_next, img_feats=image_features)

            total_loss, vis_mlm_loss, retrieval_loss, masked_lm_loss, next_sentence_loss, wra_loss = outputs
            ...
            return loss.item(), input_ids_a.size(0), wra_loss, vis_mlm_loss, retrieval_loss, mlm_loss, relation_loss
        ...
        if args.visual_learning:
            ...
            loss1, nb_tr_example1, wra_loss1, vis_mlm_loss1, retrieval_loss1, mlm_loss1, relation_loss1 = forward_backward(
                image_features=images1, input_ids_a=input_ids_a1, input_mask_a=input_mask_a1,
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            metrics_to_log = {
                ...
                'CLIP_loss': retrieval_loss1+retrieval_loss2, 'rel_loss': relation_loss1+relation_loss2}
            }
