...
from oscar.modeling.modeling_vlbert import BiImageBertForRE, BiImageBertForSequenceClassification
...
MODEL_CLASSES = {
    'bert': (BertConfig, BiImageBertForRE, BertTokenizer),
}
...
class REDataset(Dataset):
    ...
    def __init__(self, args, name, tokenizer):
        ...
        self.phrases = json.load(open(op.join(args.data_dir, 're_sg_{}.json'.format(name)), 'r'))
        self.phrases = [([i['tuple'] for i in item['test_tuples']], item['image_id']) for item in self.phrases]
        ...
        if args.det_json is not None:
            self.det_box = json.load(open(args.det_json, 'r'))
            self.use_det = True
        else:
            self.use_det = False

        # loading the referring expression data
        self.examples = json.load(open(args.data_file, 'r'))[name]
        ...
        for item in tqdm(self.examples):
            ...
            if self.use_det:
                det_boxes = [[b[0], b[1], b[0]+b[2], b[1]+b[3]] for b in self.det_box[str(img_id)]]
                d_labels = [compute_iou(n_bbox, b) for b in det_boxes]
                self.det_labels.append(d_labels)
                pred_boxes = self.od_labels[int(img_id)]['boxes']
                tmp_iou_mat = np.zeros((len(pred_boxes), len(det_boxes)))
                for i, p_b in enumerate(pred_boxes):
                    for j, d_b in enumerate(det_boxes):
                        tmp_iou_mat[i,j] = compute_iou(p_b, d_b)
                self.iou_mats.append(tmp_iou_mat)
            if len(tmp_labels) > args.max_img_seq_length:
                tmp_labels = tmp_labels[:args.max_img_seq_length]
            if self.use_det:
                if len([l for l in d_labels if l>=0.5]) > 0:
                    upper_bound += 1
            else:
    ...
    def get_det_label(self, index):
        assert(self.use_det)
        return self.det_labels[index], self.iou_mats[index]
    ...
    #for epoch in train_iterator:
        ...
        for step, (indexes, batch) in enumerate(train_dataloader):
            ...
            if args.use_det:
                ...
                tmp_labels = batch[6].cpu().numpy()
                for i in range(logits.shape[0]):
                    ind = indexes[i]
                    det_label, det_mat = train_dataset.get_det_label(ind)
                    if args.det_mod == 1:
                        batch_score += int(re_score_det1(logits[i], tmp_labels[i], det_label, det_mat)>=0.5)
                    else:
                        batch_score += int(re_score_det2(logits[i], tmp_labels[i], det_label, det_mat)>=0.5)
                batch_score = batch_score / args.train_batch_size
            else:
                tmp_score = re_score(logits, batch[6]).sum()
        ...
        #for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                if args.use_det:
                    ...
                    for i in range(logits.shape[0]):
                        ind = indexes[i]
                        det_label, det_mat = eval_dataset.get_det_label(ind)
                        if args.det_mod == 1:
                            batch_score.append(re_score_det1(logits[i], tmp_labels[i], det_label, det_mat))
                        else:
                            batch_score.append(re_score_det2(logits[i], tmp_labels[i], det_label, det_mat))
                    score += sum([int(k>=0.5) for k in batch_score])
                else:
                    batch_score = re_score(logits, batch[6])
...
def main():
    ...
    #if args.do_eval:
    eval_dataset = REDataset(args, 'val', tokenizer)

    if args.eval_test:
        testa_dataset = REDataset(args, 'testA', tokenizer)
        testb_dataset = REDataset(args, 'testB', tokenizer)
    else:
        testa_dataset, testb_dataset = None, None

    if args.do_test:
        test_dataset = REDataset(args, 'testA', tokenizer)


    # Training
    if args.do_train:
        #train_dataset = load_and_cache_examples(args, args.task_name, tokenizer, evaluate=False)
        train_dataset = REDataset(args, 'train', tokenizer)
        global_step, tr_loss = train(args, train_dataset, eval_dataset, model, tokenizer, (testa_dataset, testb_dataset))
        logger.info(" global_step = %s, average loss = %s", global_step, tr_loss)

    # Training on train+val
    if args.do_train_val:
        train_dataset = REDataset(args, 'train+val', tokenizer)
