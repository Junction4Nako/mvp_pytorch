...
class VQATextProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir, file_name='train2014_qla.json'):
        ...
        if 'train+val' in file_name:
            sgs = json.load(open(os.path.join(data_dir, 'train+val_sg_tuples.json')))
        else:
            sgs = json.load(open(os.path.join(data_dir, 'train_sg_tuples.json')))
        return self._create_examples(lines, sgs, "train")
    ...
    def get_dev_examples(self, data_dir, file_name='val2014_qla.json'):
        ...
        lines = json.load(open(os.path.join(data_dir, file_name)))
        ds_name = file_name.split('_')[0]
        sgs = json.load(open(os.path.join(data_dir, 'val_sg_tuples.json')))
        return self._create_examples(lines, sgs, "dev")
    ...
    def get_test_examples(self, data_dir, file_name='test2015_qla.json'):
        ...
        lines = json.load(open(os.path.join(data_dir, file_name)))
        ds_name = file_name.split('_')[0]
        sgs = json.load(open(os.path.join(data_dir, '{}_sg_tuples.json'.format(ds_name))))
        return self._create_examples(lines, sgs, "test")
    ...
    def _create_examples(self, lines, sgs, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            all_tuples.append(sgs[i])
        return examples, all_tuples
...
class NLVRProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir, use_label_seq=True, file_name='nlvr2_train.json'):
        ...
        lines = json.load(open(os.path.join(data_dir, file_name)))
        split = file_name[6:].split('.')[0]
        sgs = json.load(open(os.path.join(data_dir, 'nlvr_sg_{}.json'.format(split))))
        return self._create_examples(lines, "train", use_label_seq), sgs
    ...
    def get_dev_examples(self, data_dir, use_label_seq=True, file_name='nlvr2_dev.json'):
        ...
        split = file_name[6:].split('.')[0]
        sgs = json.load(open(os.path.join(data_dir, 'nlvr_sg_{}.json'.format(split))))
        return self._create_examples(lines, "dev", use_label_seq), sgs
    ...
    def get_test_examples(self, data_dir, use_label_seq=True, file_name='nlvr2_test1.json'):
        ...
        split = file_name[6:].split('.')[0]
        sgs = json.load(open(os.path.join(data_dir, 'nlvr_sg_{}.json'.format(split))))
        return self._create_examples(lines, "test", use_label_seq), sgs
