...
from oscar.modeling.modeling_vlbert import BiImageBertForSequenceClassification, BiImageBertForSequenceClassificationPlus
...
class VEDataset(Dataset):
    ...
    def __init__(self, args, name, tokenizer):
        ...
        assert name in ['train', 'val', 'test', 'train+val']
        ...
        self.examples, self.phrases = _load_dataset(args, name)
    ...
    def tensorize_example(self, example, phrases, cls_token_at_end=False, pad_on_left=False,
        ...
        tokens_a = self.tokenizer.tokenize(example['sentence2'])
        img_key = example['Flickr30K_ID']
        num_extra_tokens = 2
        ...
        if self.args.mask_training and 'train' in self.name:
            for i in range(1,len(input_ids_a)-1):
                dice = random.random()
                if dice < 0.15:
                    input_ids_a[i] = self.tokenizer.vocab['[MASK]']
        ...
        if self.args.output_mode == "classification":
            tmp_label = label_mapping[example['gold_label']]
        elif self.args.output_mode == "regression":
            raise NotImplementedError
        else:
            raise KeyError(self.args.output_mode)

        new_scores = torch.tensor(tmp_label, dtype=torch.long)

        if self.c_ids < 1 and 'train' in self.name:
            self.c_ids += 1
            logger.info("*** Example ***")
            logger.info("guid: %s" % (example['pairID']))
            ...
            logger.info("label: {}".format(tmp_label))
...
def train(args, train_dataset, eval_dataset, model, tokenizer):
    ...
    #for epoch in train_iterator:
        ...
        for step, (indexes, batch) in enumerate(train_dataloader):
            ...
            batch_score = score_with_cls_logits(logits, batch[6]).sum()
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        #for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                batch_score = score_with_cls_logits(logits, batch[6])
...
def main():
    ...
    if args.do_train_val:
        train_dataset = VEDataset(args, 'train+val', tokenizer)
        global_step, tr_loss = train(args, train_dataset, eval_dataset, model, tokenizer)
        logger.info(" global_step = %s, average loss = %s", global_step, tr_loss)
